import { useEffect, useRef, FC, useMemo, useCallback } from "react";
...
class CarLights {
  ...
  init() {
    ...
    const geometry = new THREE.TubeGeometry(
      ...
      8,
      false
    ) as unknown as THREE.InstancedBufferGeometry;
  ...
  loadAssets(): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      searchImage.addEventListener("load", function () {
        if (!assets.smaa) assets.smaa = {};
        assets.smaa.search = this as HTMLImageElement;
        manager.itemEnd("smaa-search");
      });

      areaImage.addEventListener("load", function () {
        if (!assets.smaa) assets.smaa = {};
        assets.smaa.area = this as HTMLImageElement;
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !isInitialized.current) {
            ...
            setTimeout(() => {
              const myApp = new App(container, processedOptions);
